...
END_PROVIDER
 ...
 BEGIN_PROVIDER[ double precision, aos_in_r_full_extra_grid, (ao_num,n_points_ang_extra_grid,n_points_rad_extra_grid,nucl_num) ]
  implicit none
  BEGIN_DOC
  ! MOs in r on the FULL EXTRA grid
  END_DOC
  ...
  write(*,*) 'PROVIDING aos_in_r_full_extra_grid'
  write(*,*) '... shape(aos_in_r_full_extra_grid)', shape(aos_in_r_full_extra_grid)

  aos_in_r_full_extra_grid(:,:,:,:) = 0.d0
  do i_nucl = 1, nucl_num
    do i_rad = 1, n_points_rad_extra_grid-1
      do i_ang = 1, n_points_ang_extra_grid
        r(1:3) =  grid_points_extra_per_atom(1:3,i_ang,i_rad,i_nucl)
        call give_all_aos_at_r(r,aos_in_r_full_extra_grid(:,i_ang,i_rad,i_nucl))
      end do
    end do
  end do
