...
BEGIN_PROVIDER [ double precision, core_tcxc_grid11je, (ao_num, ao_num, ao_num, ao_num)]
  implicit none
  BEGIN_DOC
  ...
  double precision, external :: j_mu_env, core_tcxc_kernel_grid11je
    ! Temporary external definition, we should put them in a module an import that

  core_tcxc_grid11je(:,:,:,:) = 0.d0
  ...
  do i1 = 1, n_points_final_grid
    ...
    do i2 = 1, n_points_final_grid
      ...
      do j = 1, ao_num
        ...
        v_x_core_ao_j_tc = core_tcxc_kernel_grid11je(r1,i2,j) 
        do l = 1, ao_num
          ...
          do k = 1, ao_num
            ...
            do i = 1, ao_num
              ...
              core_tcxc_grid11je(i,k,l,j) += w1 * ao_i_r1 * ao_k_r1 * w2 * j_factor * ao_l_r2 * v_x_core_ao_j_tc
            enddo
          enddo
        enddo
      enddo
...
!pure function core_tcxc_kernel_grid11je(r1,i2,j) result(res)
function core_tcxc_kernel_grid11je(r1,i2,j) result(res)
  BEGIN_DOC 
