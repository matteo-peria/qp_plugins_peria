...
BEGIN_PROVIDER [ double precision, core_tcxc_grid12adapt_dgemm, (ao_num, ao_num, ao_num, ao_num)]
  implicit none
  BEGIN_DOC
  ...
  core_tcxc_grid12adapt_dgemm(:,:,:,:) = 0.d0

  ao_num2 = ao_num*ao_num
  call dgemm( 'N', 'T', ao_num2, ao_num2, n_points_final_grid, 1.d0 &
            , ao_overlap_mat_grid1, ao_num2    &
            , ao_core_xc_mat_grid12adapt, ao_num2    &
            , 0.d0, core_tcxc_grid12adapt_dgemm, ao_num2 )
...
! LARGE PART OF THE CALCULATIONS
 BEGIN_PROVIDER [ double precision, core_tcxc_grid12adapt_loops, (ao_num, ao_num, ao_num, ao_num) ]
&BEGIN_PROVIDER [ double precision, ao_core_xc_mat_grid12adapt, (ao_num, ao_num, n_points_final_grid) ]
  integer :: i1, i2, i2p
    ! Grids indices
  integer :: i, j, k, l
    ! AOs indices
  ...
  ao_core_xc_mat_grid12adapt(:,:,:) = 0.d0
  core_tcxc_grid12adapt_loops(:,:,:,:) = 0.d0
        ...
        do i2p_ang = 1, n_points_ang_float_grid
          ...
          if (distance.gt.1.d-10) then
            ...
            if (core_tcxc_omp) then
              ! Since ao_core_xc_mat_grid12adapt is updated at the second nested loop,
              ! only the first two loops can be OMP-parallelised

              !$OMP PARALLEL DEFAULT(NONE) &
              !$OMP& PRIVATE (i, j, k, l, ao_core_xc_r2, v_xc_core_ao_j_r2p) &
              !$OMP& SHARED ( ao_core_xc_mat_grid12adapt                         &
              !$OMP&        , core_tcxc_grid12adapt_loops                  &
              ...
              do j = 1, ao_num
                do l = 1, ao_num
                  ...
                  ao_core_xc_mat_grid12adapt(j,l,i1) += ao_core_xc_r2

! REMOVE THIS AFTER
                  do k = 1, ao_num
                    do i = 1, ao_num
                      !core_tcxc_grid12adapt_loops(i,k,l,j) += &
                      core_tcxc_grid12adapt_loops(i,k,j,l) += &
            ...
            ! Option 2: NO parallelism 
              ...
              do l = 1, ao_num
                ...
                do j = 1, ao_num
                  ...
                  ao_core_xc_mat_grid12adapt(j,l,i1) += w2 * j_r1r2 * ao_l_r2 &
...
! THIS PROVIDER IS NOW COMPUTED TOGETHER WITH THE PROVIDER core_tcxc_gri12adapt_loops
! 
!BEGIN_PROVIDER [ double precision, ao_core_xc_mat_grid12adapt, (ao_num, ao_num, n_points_final_grid) ]
!  integer :: i1, i2, i2p
!    ! Grids indices
...
!! OMP ALTERNATIVE: 
!!!$OMP PARALLEL DO
!!!$OMP PARALLEL DO PRIVATE(ao_l_r2, ao_j_r2p) SHARED(aos_in_r_array2, aos_in_r2p, ao_core_xc_mat_grid12adapt, w2, j_r1r2, w2p, kernel, j_r1r2p, ao_num)
!!do l = 1, ao_num
!!    ao_l_r2 = aos_in_r_array2(l, i2)
!!    do j = 1, ao_num
!!        ao_j_r2p = aos_in_r2p(j)
!!        ! Notice the index order, because this array will be transposed later!
!!        ao_core_xc_mat_grid12adapt(j, l, i1) = ao_core_xc_mat_grid12adapt(j, l, i1) + w2 * j_r1r2 * ao_l_r2 * w2p * kernel * ao_j_r2p * j_r1r2p
!!    enddo
!!enddo
!!!$OMP END PARALLEL DO
!
...
! THIS PROVIDER CAN BE COMPUTED IN TWO WAYS, ONE USING LOOPS AND THE OTHER USING DGEMM
! FOR TESTING PURPOSE WE KEEP THE TWO VERSION SEPARATED
!BEGIN_PROVIDER [ double precision, core_tcxc_grid12adapt, (ao_num, ao_num, ao_num, ao_num)]
!  implicit none
!  BEGIN_DOC
!  ! Numerical evaluation of < kl | V^{\text{TC}}_{x,\text{core}}  | ij >
!  !
...
!        
!                      core_tcxc_grid12adapt(i,k,l,j) += w1 * ao_i_r1 * ao_k_r1            &
!                                                         * w2 * j_r1r2 * ao_l_r2             &
!                                                         * w2p * kernel * ao_j_r2p * j_r1r2p
!                    enddo
!                  enddo
...
!              , ao_overlap_mat_grid1, ao_num2    &
!              , ao_core_xc_mat_grid12adapt, ao_num2    &
!              , 0.d0, core_tcxc_grid12adapt, ao_num2 )
!  end if
!
!END_PROVIDER
