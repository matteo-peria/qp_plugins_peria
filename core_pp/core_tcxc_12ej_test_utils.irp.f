BEGIN_PROVIDER [ double precision, core_xpot_grid2ej_full, (ao_num, ao_num)]
  ...
  do i2 = 1, n_points_final_grid2
    ...
    ! Loop over the 3rd grid (which is the extra-full)
...
BEGIN_PROVIDER [ double precision, core_tcxc_j0_grid12ej_prun, (ao_num, ao_num, ao_num, ao_num)]
  implicit none
  BEGIN_DOC
  ! TEST-ONLY PROVIDER.
  ! Check that the TC exchange integral make sense when the Jastrow factor 
  ...
  ! Initialization
  core_tcxc_j0_grid12ej_prun(:,:,:,:) = 0.d0
  ...
  do j = 1, ao_num
    do l = 1, ao_num
      do k = 1, ao_num
        do i = 1, ao_num
          core_tcxc_j0_grid12ej_prun(i,k,l,j) = ao_overlap_grid1(i,k) * core_xpot_grid2ej_prun(j,l)
        end do
      end do
    end do
  end do
  ...
  !! vectorized version of the tensor product
  !core_tcxc_j0_grid12ej_prun = reshape( ao_overlap_grid1, shape=[ao_num,ao_num,1,1] ) &
  !                     * reshape( core_xpot_grid22, shape=[1,1,ao_num,ao_num] )
END_PROVIDER
