BEGIN_PROVIDER [ double precision, core_xpot_grid2je_prun, (ao_num, ao_num)]
  implicit none
  BEGIN_DOC
  ...
  do i2 = 1, n_points_final_grid2
    ...
    do j = 1, ao_num
      ...
      do l = 1, ao_num
        ...
        core_xpot_grid2je_prun(l,j) += w2 * ao_l_r2 * integral
      enddo
...
!BEGIN_PROVIDER [ double precision, overlap_corexc_grid12e_prun, (ao_num, ao_num, ao_num, ao_num)]
BEGIN_PROVIDER [ double precision, core_tcxc_j0_grid12je_prun, (ao_num, ao_num, ao_num, ao_num)]
  implicit none
  BEGIN_DOC
  ! TEST-ONLY PROVIDER.
  ...
  ! Initialization
  core_tcxc_j0_grid12je_prun(:,:,:,:) = 0.d0
  ...
  do j = 1, ao_num
    do l = 1, ao_num
      do k = 1, ao_num
        do i = 1, ao_num
          core_tcxc_j0_grid12je_prun(i,k,l,j) = ao_overlap_grid1(i,k) * core_xpot_grid2je_prun(j,l)
        end do
      end do
    end do
  end do
  ...
  !core_tcxc_j0_grid12je_prun = reshape( ao_overlap_grid1, shape=[ao_num,ao_num,1,1] ) &
