...
BEGIN_PROVIDER [ double precision, core_tcxc_j0_grid1eej, (ao_num, ao_num, ao_num, ao_num)]
  implicit none
  BEGIN_DOC
  ! TEST-ONLY PROVIDER.
  ...
  do j = 1, ao_num
    do l = 1, ao_num
      do k = 1, ao_num
        do i = 1, ao_num
          core_tcxc_j0_grid1eej(i,k,l,j) = ao_overlap_grid1(i,k) * core_xpot_eej(j,l)
        end do
      end do
    end do
  end do
  ...
  !core_tcxc_j0_grid1eej = reshape( ao_overlap_grid1, shape=[ao_num,ao_num,1,1] ) &
  !                     * reshape( core_xpot_eej, shape=[1,1,ao_num,ao_num] )
END_PROVIDER


BEGIN_PROVIDER [ double precision, core_xpot_eej, (ao_num, ao_num)]
  implicit none
  BEGIN_DOC
      ...
      if(distance.gt.1.d-10) then
        ...
        do j = 1, ao_num
          ...
          do l = 1, ao_num
            ...
            ! Update
            core_xpot_eej(l,j) += w2 * ao_l_r2 * w2p * kernel * ao_j_r2p / distance
          enddo
        enddo
      endif
    enddo
