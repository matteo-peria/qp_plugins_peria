subroutine get_int2_grad1_u12_ao_full_adaptive( i1, n_aos                &
  ...
  !allocate(aos_lj_prod_in_r2(ao_num, ao_num, n_float))
    ...
    do r2_ang = 1, n_points_ang_float_grid
      ...
      do l = 1, ao_num
        ...
        do j = 1, ao_num
          ...
          !aos_lj_prod_in_r2(j,l,i2) = w2 * ao_l_r2 * ao_j_r2
 ...
 do m = 1, 3
   call dgemm( "T", "N", ao_num*ao_num, 1, n_float &
             !, 1.d0, aos_lj_prod_in_r2(1,:,:), n_float      & 
   !call dgemm( "N", "N", ao_num*ao_num, 1, n_float &
             , 1.d0, aos_lj_prod_in_r2(:,:,:), n_float      & 
             , grad1_u12_vect_at_r1(:,m), n_float           &
             , 0.d0, int2_grad1_u12_ao_vect(:,:,m), ao_num*ao_num) 
 enddo
 ...
 !call dgemm( "N", "N", ao_num*ao_num, 1, n_float &
           ...
           , 0.d0, int2_grad1_u12_ao_sqrd(:,:), ao_num*ao_num) 
  ...
  do r2_nuc = 1, nucl_num                    !n_nuc_fixed
    do r2_rad = 1, n_points_rad_extra_grid-1 !n_rad_fixed-1
      do r2_ang = 1, n_points_ang_extra_grid !n_ang_fixed
        ! Index running on the total adaptive grid in r2
  ...
  enddo
  call dgemm( "T", "N", ao_num*ao_num, 1, n_fixed              &
            , -0.5d0, aos_lj_prod_in_r2(:,:,:), n_fixed        &
            , grad1_u12_sqrd_at_r1(:), n_fixed           &
            !, grad1_u12_sqrd_on_full_extra(:,i1), n_fixed        &
            , 1.d0, int2_grad1_u12_ao_sqrd(:,:), ao_num*ao_num &
           ) 
  !call dgemm( "N", "N", ao_num*ao_num, 1, n_fixed              &
  !          , -0.5d0, aos_lj_prod_in_r2(:,:,1), n_fixed        &
  !          , grad1_u12_sqrd_on_full_extra(:,i1), n_fixed        &
  !         , 1.d0, int2_grad1_u12_ao_sqrd(:,:), ao_num*ao_num &
  !        ) 
