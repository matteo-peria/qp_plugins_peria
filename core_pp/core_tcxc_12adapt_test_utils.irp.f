BEGIN_PROVIDER [ double precision, core_xpot_ao_grid2adapt, (ao_num, ao_num)]
  implicit none
  BEGIN_DOC
        ...
        if (distance.gt.1.d-10) then
          ...
          do j = 1, ao_num
            ...
            do l = 1, ao_num
              ...
              core_xpot_ao_grid2adapt(l,j) += w2 * ao_l_r2 * integral
            enddo
          enddo
...
BEGIN_PROVIDER [ double precision, core_tcxc_j0_grid12adapt, (ao_num, ao_num, ao_num, ao_num)]
  implicit none
  BEGIN_DOC
  ! TEST-ONLY PROVIDER.
  ...
  ! Initialization
  core_tcxc_j0_grid12adapt(:,:,:,:) = 0.d0

  if (core_tcxc_omp) then
    !$OMP PARALLEL DEFAULT(NONE) &
    !$OMP& PRIVATE (i, j, k, l) &
    !$OMP& SHARED ( ao_num, core_tcxc_j0_grid12adapt          &
    !$OMP&        , ao_overlap_grid1, core_xpot_ao_grid2adapt )
    !$OMP DO COLLAPSE(4) SCHEDULE(static)
    do j = 1, ao_num
      do l = 1, ao_num
        do k = 1, ao_num
          do i = 1, ao_num
            core_tcxc_j0_grid12adapt(i,k,l,j) = ao_overlap_grid1(i,k) * core_xpot_ao_grid2adapt(j,l)
          end do
        end do
      end do
    end do
